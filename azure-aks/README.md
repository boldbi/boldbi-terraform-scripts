...
Other than this, we need to add the following environment variables either as a [local system](https://chlee.co/how-to-setup-environment-variables-for-windows-mac-and-linux/) variable or in [Azure Key Vault](https://learn.microsoft.com/en-us/azure/key-vault/secrets/quick-create-portal) as shown below:
...
| TF_VAR_boldbi_unlock_key     | boldbi-unlock-key             | Yes      | **Optional:-** Provide a license key for automatic activation. If you leave this empty, you can manually upload or select a trial license in the Bold BI startup page after deployment.<br>   **Note:-** Trail license valid for 30 days alone.                            |
